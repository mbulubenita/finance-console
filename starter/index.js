...
// Function to analyze the dataset and display results on the webpage and console
function analyzeDataset(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const currentProfitLoss = currentData[1];

    totalProfitLoss += currentProfitLoss;

    if (i > 0) {
      const previousProfitLoss = data[i - 1][1];
  ...
  // Create HTML content with the calculated results
  const resultsHTML = `
    <p>Total Months: ${totalMonths}</p>
    <p>Total Profit/Loss: $${totalProfitLoss}</p>
    <p>Average Change: $${averageChange.toFixed(2)}</p>
    <p>Greatest Increase: ${greatestIncrease.date} ($${greatestIncrease.amount})</p>
    <p>Greatest Decrease: ${greatestDecrease.date} ($${greatestDecrease.amount})</p>
  `;

  // Set the innerHTML of the results element
